...
describe('Creating account tests', () => {
    ...
    it('should handle RESET', () => {
        expect(
            register({ loading: true, registrationSuccess: true, registrationFailure: true, errorMessage: '', successMessage: '', dataError: '' }, reset())
        ).toEqual({
            ...initialState,
        });
    });
    ...
    describe('Actions', () => {
        ...
        it('dispatches CREATE_ACCOUNT_PENDING and CREATE_ACCOUNT_FULFILLED actions', async () => {
            ...
            await store.dispatch(handleRegister({ firstName: '', lastName: '', login: '', email: '', password: '', image: '' }));
